  ...
  describe '#calculate_attrs' do
    ...
    describe 'with subordinately objects' do
      ...
      context 'when has many' do
        ...
        shared_examples 'balance attribute works in proper way' do
          it { expect(subject.map {|user| user.accounts.map(&:balance).sort } ).to eq [[100, 200, 300], [1000, 2000, 3000], [10000, 20000, 30000]] }
          it { expect(lambda { subject.load }).to be_executed_sqls(3) }
        end

        shared_examples 'balance and number_of_transactions attributes in proper way' do
          it_behaves_like 'balance attribute works in proper way'
          it { expect(subject.map {|user| user.accounts.map(&:number_of_transactions).sort }).to eq [[10, 20, 30], [10, 20, 30], [10, 20, 30]] }
        end

        context 'one attribute' do
          subject { User.includes(:accounts).calculate_attrs(accounts: :balance).order(:id) }
          it_behaves_like 'balance attribute works in proper way'
        end

        context 'two attributes' do
          subject { User.includes(:accounts).calculate_attrs(accounts: [:balance, :number_of_transactions]).order(:id) }
          it_behaves_like 'balance and number_of_transactions attributes in proper way'
        end

        context 'true as parameter' do
          subject { User.includes(:accounts).calculate_attrs(accounts: true).order(:id) }
          it_behaves_like 'balance and number_of_transactions attributes in proper way'
        end

        context 'no parameters' do
          subject { User.includes(:accounts).calculate_attrs(:accounts).order(:id) }
          it_behaves_like 'balance and number_of_transactions attributes in proper way'
        end
      end
